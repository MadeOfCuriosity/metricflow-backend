...
class RoomService:
    ...
    def create_room(
        ...
        if data.parent_room_id:
            ...
            # Prevent nested sub-rooms: parent must be a top-level room
            if parent.parent_room_id is not None:
                raise ValueError("Cannot create nested sub-rooms. Sub-rooms can only be created under top-level rooms.")
    ...
    def get_room_kpis(
        ...
        """
        Get KPIs for a room.
        Returns (room_specific_kpis, sub_room_kpis, shared_kpis).
        For top-level rooms, sub_room_kpis contains KPIs from all sub-rooms.
        Sub-rooms only see their own KPIs (no parent inheritance).
        """
        ...
        # Get sub-room KPIs (for top-level rooms only)
        sub_room_kpis = []
        if is_top_level:
            # Find all sub-rooms of this room
            sub_room_ids = [
                sr_id for (sr_id,) in db.query(Room.id).filter(
                    Room.parent_room_id == room_id
                ).all()
            ]
            if sub_room_ids:
                # Get all KPI IDs from sub-rooms, excluding already-assigned ones
                sub_kpi_ids = [
                    kpi_id for (kpi_id,) in db.query(RoomKPIAssignment.kpi_id).filter(
                        RoomKPIAssignment.room_id.in_(sub_room_ids),
